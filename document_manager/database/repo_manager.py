...
def createRepo(repoDirPath, name, layerNames=[], headers=[], counter=0, fields=[], documentSets=[]):
	if repoDirPath[-1] != "/" and repoDirPath[-1] != "\\":
		repoDirPath += "/"

	# Check for prior existance
	if os.path.exists(repoDirPath):
		debug("ERROR: RepoDirPath already exists before creation: " + repoDirPath)
		return False

	# Create directories
	os.makedirs(repoDirPath)
	os.mkdir(repoDirPath + "repo-documents")

	# Create repo-config
	with open(repoDirPath + "repo-config", "w+") as f:
		f.write("#Name")
		f.write(name)
		f.write("#LayerNames")
	...
	# Create repo-data
	with open(repoDirPath + "repo-data", "w+") as f:
		f.write("#Counter")
		f.write(counter)
		f.write("#Fields")
		for field in fields:
			f.write(str(field))
		f.write("#DocumentSets")
		for documentSet in documentSets:
			f.write(str(documentSet))

	return loadRepo(repoDirPath)


def loadRepo(repoDirPath):
	pass
